...
function Checkout() {
    ...
    const {  showService, showProducts, menu, totals, customer, setCustomer,car } = useContext(StoreContext);
    const navigate = useNavigate();

    function onChangeHandler(event: any) {
        setCustomer((pre: any) => ({ ...pre, [event.target.name]: event.target.value }));
    }
                                    ...
                                    <div className="mb-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                ...
                                                <input name="email" type="email" id="last_name" className="w-full rounded-lg border py-2 px-3" value={customer.email} onChange={() => onChangeHandler(event)} />
                                            </div>
                                        </div>
                                        <div className="mt-4">
                                            <label htmlFor="address" className="block text-gray-700   mb-1">Address</label>
                                            <textarea name="address" id="address" className="w-full rounded-lg  py-2 px-3 border-2" value={customer.address} onChange={() => onChangeHandler(event)} />
                                        </div>

                                        <div className="mt-4">
                                            <label htmlFor="city" className="block text-gray-700   mb-1">City</label>
                                            <input name="city" type="text" id="city" className="w-full rounded-lg  py-2 px-3 border-2" value={customer.city} onChange={() => onChangeHandler(event)} />
                                        </div>

                                        <div className="grid grid-cols-2 gap-4 mt-4">
                                            <div>
                                                <label htmlFor="state" className="block text-gray-700   mb-1">State</label>
                                                <input name="state" type="text" id="state" className="w-full rounded-lg  py-2 px-3 border-2" value={customer.state} onChange={() => onChangeHandler(event)} />
                                            </div>
                                            <div>
                                                <label htmlFor="zip" className="block text-gray-700   mb-1">ZIP Code</label>
                                                <input name="zip" type="text" id="zip" className="w-full rounded-lg  py-2 px-3 border-2" value={customer.zip} onChange={() => onChangeHandler(event)} />
                                            </div>
                                        </div>
                                    </div>
                                </div>

                        ...
                        <>
                            ...
                            <div className="flex flex-col mt-5 justify-center md:flex-row gap-3">
                                <select id="status" name="status" onChange={() => paymentsFilter(event)}
                                    ...
                                    <option className="dark:text-black" value="succeeded">Succeess</option>
                                    <option className="dark:text-black" value="requires_payment_method">Fail</option>
                                </select>
                            </div>
                            {
                            filterPayments?.length>0?
                                <div className={`shadow-lg rounded-lg overflow-hidden my-10 mx-10 overflow-y-auto max-h-[500px]`}>
                                <table className="w-full table-fixed" >
                                    <thead>
                                        <tr className="bg-gray-100 dark:bg-white/10 dark:text-white text-gray-600">
                                            <th className="w-1/4 py-4 px-6 text-left  font-bold uppercase">No</th>
                                            <th className="w-1/4 py-4 px-6 text-left  font-bold uppercase">Payment Id</th>
                                            <th className="w-1/4 py-4 px-6 text-left  font-bold uppercase">Amount</th>
                                            <th className="w-1/4 py-4 px-6 text-left  font-bold uppercase">Status</th>
                                            <th className="w-1/4 py-4 px-6 text-left  font-bold uppercase">View Receipt</th>
                                        </tr>
                                    </thead>
                                    ...
                                    <tbody className="bg-white dark:bg-white/40 dark:text-white ">
                                        {
                                            filterPayments.map((value: any, index:any) => {
                                                return (
                                                    <tr key={value.id} className="text-black ml-8">
                                                        <td className="py-4 px-6 border-b border-gray-200">{index + 1}</td>
                                                        <td className="py-4 px-6 border-b border-gray-200">{value.id}</td>
                                                        <td className="py-4 px-6 border-b border-gray-200">{value.amount}</td>
                                                        <td className="py-4 px-6 border-b border-gray-200">{value.status}</td>
                                                        <td className="py-4 px-6 border-b border-gray-200">
                                                            {value.receiptURL ?
                                                                <button className="bg-green-500 text-white py-1 px-2 rounded-full hover:bg-green-300 hover:text-black">
                                                                    <a href={value.receiptURL} target="_blank" rel="noopener noreferrer">View Receipt</a>
                                                                </button>:<span className="text-center md:ml-10">-</span>
                                                            }
                                                        </td>
                                                    </tr>
                                                )

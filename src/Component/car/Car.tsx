...
function Car() {
    ...
    const { menu, showProducts, setMenu, load, pageLoad, setPageLoad, setHeaderLoad, setFooterLoad } = useContext(StoreContext);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setIndex((pre: any) => {
                return pre == car?.images.length - 1 ? 0 : (pre + 1) % car?.images.length;
            })
        }, 5000)
        ...
        <>
            {
                ...
                <div className={`relative `}>
                    ...
                    {
                        <div className={`${showProducts | menu | load && 'blur-sm'}`}>
                            {
                                pageLoad ?
                                                            ...
                                                            <div className="flex gap-4 py-4 justify-center overflow-x-auto">
                                                                {
                                                                    car.images.map((image: any, Index: any) =>
                                                                        <img key={Index} src={image} alt="Thumbnail 1"
                                                                            className={`size-16 sm:size-20 object-cover rounded-md cursor-pointer  ${Index == index ? 'opacity-100' : 'opacity-50 dark:opacity-30'} transition duration-300`}
                                                                            onMouseEnter={() => { setIndex(Index % car.images.length) }}
                                                                        />
                                                                    )
                                                                }
                                                            </div>
